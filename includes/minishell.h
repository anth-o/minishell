...
/*   Updated: 2026/02/18 20:12:07 by ko-mahon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_cmd
{
	...
	char    *outfile;  /* for > redirection (last) */
	int     append;    /* 1 if >> */
	int     heredoc;   /* 1 if << */
	struct s_cmd *next;
}               t_cmd;
...
typedef struct s_env
{
	...
	typedef struct s_cmd
	{
		char **args;        // ex : {"ls", "-l", NULL}
		char *infile;       // pour < redirection
		char *outfile;      // pour > redirection (dernier)
		t_outredir  *outfiles;      // liste des redirections de sortie
		int append;         // 1 si >> (dernier)
		int heredoc;        // 1 si <<
		struct s_cmd *next; // commande suivante si pipe
	}                   t_cmd;
void				free_env(t_env *env);
